...
from numpy import ndarray, array, vstack, empty, maximum, concatenate, clip, broadcast_to
...
class Optimizer:
    def __init__(self,
            function: Callable,  # 目标函数：输入.shape==(D,)的ndarray，输出float
            bounds__,            # 决策变量边界
            T: int = 1000,       # 迭代次数
            N: int = 200,        # 种群规模
            n_jobs: int = 1,
            ...
            reuse_parallel: bool = True,
            ):
        assert callable(function), '目标函数function应为可调用对象'
        bounds__ = array(bounds__)  # 决策变量边界
        assert bounds__.ndim==2 and bounds__.shape[1]==2, '决策变量边界bounds__应可转化成满足.shape[1]==2的2维ndarray'
        ...
        self.batch_size = 'auto'    # joblib并行执行batch_size
        self.boundaryHandlingMethod = 'reflect'  # 边界处理方法 reflect / clip
        self.samplingMethod = 'LHS'  # 采样方法 'LHS'/'BCM'
        ...
        self.reuse_parallel = reuse_parallel
        self.parallel = None
        if n_jobs!=1 and reuse_parallel:
    ...
    def batchObjective(self,
        ...
        elif self.reuse_parallel:
            y_ = self.parallel(delayed(self.function)(x_) for x_ in X__)
        else:
            parallel = Parallel(
                n_jobs=self.n_jobs,
                batch_size=self.batch_size,
    ...
    def initialize(self, X__: ndarray | None = None):
        """初始化种群"""
        sampler = getattr(self, f'sampler{self.samplingMethod}')
        if X__ is None:
            X__ = sampler(self.N)  # 随机初始化种群 (N, D)
        else:
            assert isinstance(X__, ndarray) and X__.ndim==2 and X__.shape[1]==self.D, \
    ...
    def samplerLHS(self, N):
        ...
        sampler = LatinHypercube(d=self.D)  # 拉丁超立方采样器
    ...
    def boundaryHandling(self,
                         Xnew__: ndarray,  # (N, SE, D) or (N, D) 新种群
                         Xold__: ndarray,  # (N, 1, D) or (N, D) 原种群
                         ):
        """边界条件"""
        bounds__ = self.bounds__
        if self.boundaryHandlingMethod=='reflect':
            # 随机反射
            shape = Xnew__.shape
            Xold_broadcast__ = broadcast_to(Xold__, shape)  # 若Xold__与Xnew__不同shape，广播使同shape
            lb__ = broadcast_to(bounds__[:, 0], shape)
            ub__ = broadcast_to(bounds__[:, 1], shape)
            logic__ = Xnew__>ub__
            Xnew__[logic__] = uniform(Xold_broadcast__[logic__], ub__[logic__])
            logic__ = Xnew__<lb__
            Xnew__[logic__] = uniform(lb__[logic__], Xold_broadcast__[logic__])
        elif self.boundaryHandlingMethod=='clip':
            # 裁剪
            Xnew__ = clip(Xnew__, bounds__[:, 0], bounds__[:, 1])  # 边界处理
        else:
            raise ValueError(f'未定义边界处理方法"{self.boundaryHandlingMethod}"')
        return Xnew__

    def record(self, X__: ndarray, y_: ndarray, t: int | None = None):
        """更新全局最优、记录迭代过程的目标函数值"""
        i = y_.argmin()
    ...
    def resample(self, X__: ndarray):
        """重采样生成新种群"""
        Xmin_, Xmax_ = X__.min(axis=0), X__.max(axis=0)      # 种群D个维度的最小值、最大值
        lb_, ub_ = self.bounds__[:, 0], self.bounds__[:, 1]  # 种群D个维度的下界、上界
        N, D = X__.shape
        X__ = empty(X__.shape)  # 初始化
        # 计算左右未探索区的宽度
        leftWidths_ = maximum(Xmin_ - lb_, 0)
        rightWidths_ = maximum(ub_ - Xmax_, 0)
        totalWidths_ = leftWidths_ + rightWidths_
        for d in range(D):
            if totalWidths_[d]<1e-10:
                X__[:, d] = uniform(lb_[d], ub_[d], N)
            else:
                # 按宽度比例分配采样数量
                Nleft = int(N*leftWidths_[d]/totalWidths_[d])
                Nright = N - Nleft
                # 在左右区域分别采样
                Xleft_ = uniform(lb_[d], Xmin_[d], Nleft)
                Xright_ = uniform(Xmax_[d], ub_[d], Nright)
                X__[:, d] = concatenate([Xleft_, Xright_])
                shuffle(X__[:, d])  # 打乱顺序
        return X__
